...
//      std::string path = "thegamesdb.net/api/GetGame.php?";
  std::string path = "www.games-db.info/api/emulationstationarcademyscraper.php?";

        //std::string cleanName = params.game->getName();
        ...
        LOG(LogWarning) << "Arcademy path " << path;
        ...
        if(params.system->getPlatformIds().empty())
        {
                ...
                for(auto platformIt = platforms.begin(); platformIt != platforms.end(); platformIt++)
                {
                        ...
                        if(mapIt != gamesdb_platformid_map.end())
                        {
                                ...
                                LOG(LogWarning) << "Arcademy scraper warning - no support for platform " << getPlatformName(*platformIt);
                        }
...
void TheGamesDBRequest::process(const std::unique_ptr<HttpReq>& req, std::vector<ScraperSearchResult>& results)
{
        ...
        {
                std::stringstream ss;
                ss << "ArcademyDBRequest - Error parsing XML. \n\t" << parseResult.description() << "";
...
// nom de la rom ./xxxxx.zip
    //result.mdl.set("path", game.child("GameTitle").text().get());
